...
try {
    ...
    $WebDriver->get('https://www.routeone.co.uk/collections/fleeces');

    // Wait for the items to load
    $WebDriver->wait($TestConfig->getTimeoutSeconds())->until(
        WebDriverExpectedCondition::visibilityOfElementLocated(
            WebDriverBy::cssSelector('ul > .ais-Hits-item')
        )
    );

    $WebDriver->manage()->addCookie(
        new Facebook\WebDriver\Cookie(
            '2c.dc', 
            '%7B%225ee8d59360b27a41790c2181%22%3A%7B%22state%22%3A%22closed%22%7D%7D'
        )
    );
    ...
    for ($i=1; $i < 6; $i++) {
        ...
        $elements = $WebDriver->findElements(
            WebDriverBy::cssSelector(
                "ul > .ais-Hits-item:not(.collection-featured-product__original-product--4) h2 a"
            )
        );
        ...
        $WebDriver->wait($TestConfig->getTimeoutSeconds())->until(
            WebDriverExpectedCondition::visibilityOfElementLocated(
                WebDriverBy::cssSelector('.product__images img')
            )
        );
        sleep(2);
